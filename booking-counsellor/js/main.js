...
function toggle_reservation_time(id, week, day, time, from)
{
	if(session_user_is_admin == '1')
	{
		if(week < global_week_number || week == global_week_number && day < global_day_number)
		{
		    ...
		    var backward_time = confirm('Confirm that you want to reserving back in time. Are you sure you want to do this?');
	...
	var action;
	if(user_name == ''){
	  action = 'You Cannot Book Yourself.';
	  //return 
	  }else{
	  action = 'Confirm You Want To: Cancel Booking';
	  }
	
	var book_conf = confirm(' ' + action + '  ' );
			
	 if(book_conf){		
			//notify('User Allowed This Action', 4);
			}else{
			notify('User Cancelled This Action', 4);
			return false;
	   }
	...
	else
	{
		if(offclick_event == 'mouseup' || from == 'details')
		{
			...
			/*var confirm_book = confirm('Confirm you want to remove this booking.');
